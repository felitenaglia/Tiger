...
fun simpleVar(acc, nivel) =
     case acc of 
      ...
      | InFrame k => let fun aux 0 = TEMP fp
                     ...
                     in Ex (MEM (BINOP (PLUS, aux (!actualLevel - nivel), CONST k))) end
...
fun fieldVar(var, field) = 
  ...
  in Ex (ESEQ (seq[
           ...
           MEM (BINOP (PLUS, TEMP t, CONST (field*wSz)))))
  end
...
fun subscriptVar(arr, ind) =
  ...
  in Ex (ESEQ (seq[
           MOVE (TEMP tv, v'),
           MOVE (TEMP te, e'),
           EXP (externalCall("_checkIndexArray",[TEMP tv, TEMP te]))],
           MEM (BINOP (PLUS, TEMP tv, case e' of 
                                         CONST k => CONST (k*wSz)
                                       | _ => BINOP (LSHIFT, e', CONST log2WSz)))))
  end
...
fun recordExp l =
	...
	in Ex (ESEQ (seq(lexps'@[EXP (externalCall("_allocRecord",CONST (length l)::(map #3 l'))), MOVE (TEMP ret,TEMP rv)]),TEMP ret)) end
...
fun ifThenExp{test, then'} =
	...
	in
	   Nx (seq[(unCx test)(t,f),LABEL t, unNx then', LABEL f]) 
	end

fun ifThenElseExp {test,then',else'} =
	let 
	   val (t,f,lsal,res) = (newlabel(), newlabel(), newlabel(), newtemp())
	in
	   Ex (ESEQ( seq [(unCx test)(t,f),
	                  LABEL t,
	                  MOVE (TEMP res, unEx then'),
                    JUMP (NAME lsal,[lsal]),
	                  LABEL f,
	                  MOVE (TEMP res, unEx else'),
                    LABEL lsal], TEMP res))
	end

fun ifThenElseExpUnit {test,then',else'} =
	let 
	   val (t,f,lsal) = (newlabel(), newlabel(),newlabel())
	in
	...
	in case oper of
	      PlusOp => Ex (BINOP (PLUS, l,r))
   	    ...
   	    | DivideOp => Ex (BINOP (DIV, l,r))
            | _ => raise Fail "binOpIntExp: No es operador aritmético\n"       	    
    end
...
fun binOpIntRelExp {left,oper,right} =
	...
	in case oper of
	      EqOp => Cx (subst EQ)
   	    ...
   	    | GtOp => Cx (subst GT)
   	    | GeOp => Cx (subst GE)
        | _ => raise Fail "binOpIntRelExp: No es operador de relación\n"
    end
...
fun binOpStrExp {left,oper,right} =
	...
	in case oper of
	      EqOp => Cx (subst EQ)
   	    ...
   	    | GtOp => Cx (subst GT)
   	    | GeOp => Cx (subst GE)
        | _ => raise Fail "binOpStrExp: No es operador para strings\n"
    end
