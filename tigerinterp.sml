...
struct
	...
	fun inter showdebug (funfracs: (tigerframe.frame*stm list) list) (stringfracs: (tigertemp.label*string) list) =
	let
		(* Memoria y registros *)
		local
		...
		and evalFun(f, args) =
			let
				...
				val ffrac = List.filter (fn (frame, body) => tigerframe.name(frame)=f) funfracs
				val _ = if (List.length(ffrac)<>1) then raise Fail ("No se encuentra la función, o repetida: "^f^"\n") else ()
				val [(frame, body)] = ffrac
	...
	in (print("Comienzo de ejecución...\n"); evalFun("_tigermain", [0]); print("Fin de ejecución.\n")) end
end
