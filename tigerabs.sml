...
fun printTy (NameTy s) = "'"^s^"'"
  | printTy (ArrayTy s) = "Arr "^s
  | printTy (RecordTy l) = "Rec("^printFlds l^")"
and printFlds l = List.foldl (fn(x,y) => x^", "^y) "" (map (fn({name,typ,...})=>name^":"^printTy typ) l)
end
